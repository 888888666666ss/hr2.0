...
export async function GET(request: NextRequest) {
  try {
    ...
    let filteredReqs = mockJobReqs
    
    // 如果有状态过滤
    if (status) {
      filteredReqs = mockJobReqs.filter(req => req.status.toLowerCase() === status.toLowerCase())
    }
    
    // 模拟延迟
    await new Promise(resolve => setTimeout(resolve, 200))
    
    return NextResponse.json(filteredReqs)
  } catch (error) {
    console.error('Error fetching job reqs:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error creating job req:', error)
    return NextResponse.json(
